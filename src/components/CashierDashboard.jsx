...
// ---------- Stat Card (updated with .card & .card-body) ----------
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const todayRes = await axios.get(`${API_BASE}/closing`, {
          headers: { Authorization: `Bearer ${token}` },
          params: { cashierId, startDate: today, endDate: today },
        });
        setTodayClosing(todayRes.data.length > 0 ? todayRes.data[0] : null);
        setTodayCount(todayRes.data.length);

        const allRes = await axios.get(`${API_BASE}/closing`, {
          headers: { Authorization: `Bearer ${token}` },
          params: { cashierId },
        });
        setRecentReports(allRes.data.slice(0, 5));

        // Stats for current vs previous month
        const now = new Date();
        ...
        const [currentMonthData, prevMonthData] = await Promise.all([
          axios.get(`${API_BASE}/closing`, {
            headers: { Authorization: `Bearer ${token}` },
            params: { cashierId, startDate: format(currentMonthStart, 'yyyy-MM-dd'), endDate: format(currentMonthEnd, 'yyyy-MM-dd') },
          }),
          axios.get(`${API_BASE}/closing`, {
            headers: { Authorization: `Bearer ${token}` },
            params: { cashierId, startDate: format(prevMonthStart, 'yyyy-MM-dd'), endDate: format(prevMonthEnd, 'yyyy-MM-dd') },
          }),
        ]);
        ...
        const weeklyRes = await axios.get(`${API_BASE}/closing`, {
          headers: { Authorization: `Bearer ${token}` },
          params: { cashierId, startDate: dayStrings[0], endDate: dayStrings[6] },
        });
  ...
  return (
    <div className="p-3 sm:p-4 md:p-6 lg:p-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3 sm:gap-4 lg:gap-6 mb-6 sm:mb-8">
        {statsWithToday.map((stat, i) => <StatCard key={i} {...stat} />)}
      </div>
            ...
            {loading ? (
              ...
              <div>
                <div className="grid grid-cols-2 gap-3 sm:gap-4">
                  ...
                  <div><p className="text-xs sm:text-sm text-gray-500">Status</p><span className={`badge ${todayClosing.isShort ? 'badge-danger' : todayClosing.isExcess ? 'badge-warning' : 'badge-success'}`}>{todayClosing.isShort ? 'Short' : todayClosing.isExcess ? 'Excess' : 'Match'}</span></div>
                </div>
                <div className="mt-4 flex flex-wrap gap-2">
                  {todayClosing.notified && <span className="badge badge-warning">🔔 Notified</span>}
                  {todayClosing.charged && <span className="badge badge-danger">💳 Charged</span>}
                </div>
              </div>
            ) : (
              <p className="text-gray-500 text-sm">No closing for today. <a href="/closing" className="text-primary hover:underline">Add now →</a></p>
            )}
          </div>
        </div>
        ...
        <div className="card">
          <div className="card-body">
            <div className="flex items-center justify-between mb-3 sm:mb-4">
              ...
              <a href="/all-reports" className="text-sm text-primary hover:underline font-medium">View all</a>
            </div>
            {loading ? (
              <div className="flex justify-center py-6"><div className="spinner" /></div>
                ...
                {recentReports.map((r) => (
                  <div key={r._id} className="flex items-center justify-between p-2 sm:p-3 bg-gray-50 rounded-lg">
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-gray-800">{format(new Date(r.date), 'dd MMM yyyy')}</p>
                      <p className="text-xs text-gray-500 truncate">Bill: ₹{r.billAmount.toFixed(2)}</p>
                    </div>
                    <div className="text-right flex-shrink-0 ml-2">
                      ...
                      <span className={`badge ${r.isShort ? 'badge-danger' : r.isExcess ? 'badge-warning' : 'badge-success'}`}>{r.isShort ? 'Short' : r.isExcess ? 'Excess' : 'Match'}</span>
                    </div>
                  </div>
                ))}
              </div>
