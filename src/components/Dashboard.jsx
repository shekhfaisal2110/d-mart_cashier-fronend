...
const SidebarItem = ({ icon, label, active, onClick }) => (
  <button
    ...
    className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${
      active ? 'bg-red-50 text-red-600 font-medium' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-800'
    }`}
  >
    {icon}
      ...
      <main className="flex-1 overflow-y-auto w-full">
        ...
        <header className="bg-white border-b border-gray-200 sticky top-0 z-10">
          <div className="px-3 sm:px-4 md:px-6 lg:px-8 py-2 sm:py-3 md:py-4 flex items-center justify-between">
            ...
            {/* Right side - Install App, Notification & Profile */}
            <div className="flex items-center gap-1 sm:gap-2 md:gap-4">
              ...
              <div className="relative" ref={dropdownRef}>
                ...
                >
                  <div className="w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 rounded-full bg-red-100 flex items-center justify-center text-red-600 font-semibold text-xs sm:text-sm group-hover:bg-red-200 transition">
                    {initials || 'CA'}
                  </div>
                  <div className="hidden sm:block text-left">
                    <p className="text-xs md:text-sm font-medium text-gray-800 group-hover:text-red-600 transition truncate max-w-[80px] md:max-w-[120px]">
                      {userName}
                    </p>
                ...
                {dropdownOpen && (
                  <div className="absolute right-0 mt-2 w-64 sm:w-72 bg-white rounded-2xl shadow-2xl border border-gray-100 py-2 z-20 overflow-hidden">
                    {/* Profile Header */}
                    <div className="px-4 py-4 border-b border-gray-100 bg-gradient-to-r from-red-50 to-red-50/30">
